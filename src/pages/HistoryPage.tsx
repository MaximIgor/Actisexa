...
const TransactionHistory = ({ className, onBack }: TransactionHistoryProps) => {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground">
            <NavBar />
            <div className={cn("max-w-md mx-auto bg-white py-10 px-4", className)}>
                ...
                <div className="flex border-b mb-4 justify-between">
                    {["Recharge", "Withdrawal", "Exchange", "Changes"].map((tab) => (
                        <button
                            key={tab}
                            style={{ fontSize: "12px" }}
                            className={`py-1 px-2 flex-shrink-0 ${activeTab === tab
                                ? "bg-cyan-500 text-white rounded-t-md"
